...
static const GLenum enumToGLBufferUses[] = {GL_STATIC_DRAW,GL_DYNAMIC_DRAW};
...
//framebuffer formats
static const GLenum* enumToGLFrameBufferFormats = enumToGLTextureFormats;	//this shouldn't index past GL_SRGB8_ALPHA8 from the list above
																			//maybe create a separate array for it?
//tied to Pixel::Format
static const GLenum enumToGLPixelDataFormats[] = {GL_RED, GL_RG, GL_RGB, GL_BGR, GL_RGBA,	GL_BGRA,GL_DEPTH_COMPONENT,GL_DEPTH_STENCIL};

//tied to Pixel::Type
static const GLenum enumToGLPixelDataTypes[] = {GL_UNSIGNED_BYTE, GL_BYTE, GL_UNSIGNED_SHORT, GL_SHORT, GL_UNSIGNED_INT, GL_INT, GL_HALF_FLOAT, GL_FLOAT, GL_UNSIGNED_BYTE_3_3_2, GL_UNSIGNED_BYTE_2_3_3_REV, GL_UNSIGNED_SHORT_5_6_5, GL_UNSIGNED_SHORT_5_6_5_REV, GL_UNSIGNED_SHORT_4_4_4_4, GL_UNSIGNED_SHORT_4_4_4_4_REV, GL_UNSIGNED_SHORT_5_5_5_1, GL_UNSIGNED_SHORT_1_5_5_5_REV, GL_UNSIGNED_INT_8_8_8_8, GL_UNSIGNED_INT_8_8_8_8_REV, GL_UNSIGNED_INT_10_10_10_2, GL_UNSIGNED_INT_2_10_10_10_REV, GL_UNSIGNED_INT_24_8, GL_FLOAT_32_UNSIGNED_INT_24_8_REV};

static const GLenum enumToGLTextureParamName[] = {GL_TEXTURE_BASE_LEVEL, GL_TEXTURE_BORDER_COLOR, GL_TEXTURE_COMPARE_FUNC, GL_TEXTURE_COMPARE_MODE, GL_TEXTURE_LOD_BIAS, GL_TEXTURE_MIN_FILTER, GL_TEXTURE_MAG_FILTER, GL_TEXTURE_MIN_LOD, GL_TEXTURE_MAX_LOD, GL_TEXTURE_MAX_LEVEL, GL_TEXTURE_SWIZZLE_R, GL_TEXTURE_SWIZZLE_G, GL_TEXTURE_SWIZZLE_B, GL_TEXTURE_SWIZZLE_A, GL_TEXTURE_SWIZZLE_RGBA, GL_TEXTURE_WRAP_S, GL_TEXTURE_WRAP_T, GL_TEXTURE_WRAP_R};
static const GLenum enumToGLTextureParams[] = {GL_LEQUAL,GL_GEQUAL,GL_LESS,GL_GREATER,GL_EQUAL,GL_NOTEQUAL,GL_ALWAYS,GL_NEVER,GL_COMPARE_REF_TO_TEXTURE,GL_NONE, GL_NEAREST, GL_LINEAR, GL_NEAREST_MIPMAP_NEAREST, GL_LINEAR_MIPMAP_NEAREST, GL_NEAREST_MIPMAP_LINEAR, GL_LINEAR_MIPMAP_LINEAR, GL_RED, GL_GREEN, GL_BLUE, GL_ALPHA, GL_ZERO, GL_CLAMP_TO_EDGE, GL_CLAMP_TO_BORDER, GL_MIRRORED_REPEAT, GL_REPEAT, GL_CLAMP_TO_EDGE};

static const GLenum enumToGLDrawBufferTypes[] = {GL_NONE, GL_FRONT_LEFT, GL_FRONT_RIGHT, GL_BACK_LEFT, GL_BACK_RIGHT, GL_FRONT, GL_BACK, GL_LEFT, GL_RIGHT, GL_FRONT_AND_BACK};
static const GLenum enumToGLDrawBuffersTypes[] = {GL_NONE, GL_FRONT_LEFT, GL_FRONT_RIGHT, GL_BACK_LEFT, GL_BACK_RIGHT, GL_COLOR_ATTACHMENT0, GL_COLOR_ATTACHMENT1, GL_COLOR_ATTACHMENT2, GL_COLOR_ATTACHMENT3, GL_COLOR_ATTACHMENT4, GL_COLOR_ATTACHMENT5, GL_COLOR_ATTACHMENT6, GL_COLOR_ATTACHMENT7};

//blend
static const GLenum enumToGLBlendFunc[] = {
	GL_FUNC_ADD, GL_FUNC_SUBTRACT, GL_FUNC_REVERSE_SUBTRACT, GL_MIN, GL_MAX
};

static const GLenum enumToGLBlendParam[] = {
	GL_ZERO, GL_ONE, 
...
void ActiveTexture(Texture::Unit unit)  
{
...
bool CreateBaseTexture(Texture::SObject& obj, const Texture::SDescriptor& desc)
{
	Pixel::Data data = {Pixel::RGB, Pixel::UCHAR, 0, 0};
	if(IS_DEPTH_TEXTURE(desc.fmt)){
		data.fmt = Pixel::DEPTH_COMPONENT;
	}
	else if(IS_DEPTH_STENCIL_TEXTURE(desc.fmt)){
		data.fmt = Pixel::DEPTH_STENCIL;
		if(desc.fmt == Texture::DEPTH_STENCIL){			
			data.type = Pixel::UINT_24_8;
		}
		else if(desc.fmt == Texture::DEPTH32F_STENCIL8)
			data.type = Pixel::UINT_32_8;
		else
			data.type = Pixel::UINT_24_8;
	}
...
bool CreateBaseTextureData(Texture::SObject& obj, const Texture::SDescriptor& desc, Pixel::Data& data)
{
...
bool CreateTextureData(Texture::SObject& obj, const Texture::SDescriptor& desc, const Pixel::Data& data)
{
	...
	if(desc.nSamples == 0)
	{
		...
		{	//try just binding it once
			glBindTexture(GL_TEXTURE_BUFFER, obj.handle);
			glBindTexture(GL_TEXTURE_BUFFER, 0);
		}
		//nor for multisamples texturesz
		//http://www.opengl.org/sdk/docs/man4/xhtml/glGenerateMipmap.xml
		if(desc.genMipMap == Texture::GEN_MIPMAP)
...
void DeleteTexture(Texture::SObject tx)
{
...
void SetAttributeFormat( const VertexAttribute* pAttr, unsigned int numAttr, unsigned int v_offset)
{
	...
	unsigned int currentVBO = GetCurrentBuffer(ARRAY);
	for(unsigned int i=0; i<numAttr; i++)
	{
		if(pAttr[i].vbo != 0)
			BindBuffer(ARRAY, pAttr[i].vbo);
...
unsigned int CreateBuffer(Buffer type, BufferUse use, unsigned int size, const void* pData)
{
	unsigned int buffer=0;
	glGenBuffers(1, &buffer);
...
void UpdateBuffer(Buffer idx, BufferUse uidx, unsigned int id, unsigned int size, const void * pData, unsigned int offset)
{
	...
	glBindBuffer(enumToGLBuffers[idx], 0);
}
...
void CreateFrameBuffer(FrameBuffer::SObject& fbo)
{
	...
	memset(fbo.attachment,0,sizeof(SAttachment)*(ATT_CLR_MAX+1));
...
void AddRenderBuffer(FrameBuffer::SObject& fbo, const RenderBuffer::SObject& rbo, const AttachmentTarget idx)
{
	...
	if(IS_COLOR_RENDERBUFFER(rbo.descriptor.fmt))
	{
		if(idx >= ATT_DEPTH)
		{
			d_printf("can't bind colour texture to depth component\n");
			return;
		}

		fbo.attachment[idx].type = SAttachment::RenderBuffer;
		//fbo.attachment[fbo.numColourAttachments].idx = idx;
		fbo.attachment[idx].active = true;	
		fbo.attachment[idx].r = rbo;
		//bind renderbuffer	to the framebuffer
		glFramebufferRenderbuffer(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0+idx, GL_RENDERBUFFER, rbo.handle);		
	}
	else if(IS_DEPTH_RENDERBUFFER(rbo.descriptor.fmt))
	{
		//maybe check if it's already active?
		fbo.depth_attachment.type = SAttachment::RenderBuffer;
		fbo.depth_attachment.active = true;	
		fbo.depth_attachment.r = rbo;
		glFramebufferRenderbuffer(GL_DRAW_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, rbo.handle);				
	}
	//TODO
...
void AddTextureRenderBuffer(FrameBuffer::SObject& fbo, const Texture::SObject& to, const AttachmentTarget idx, int mipmaplevel)
{
	...
	if(IS_COLOR_TEXTURE(to.descriptor.fmt))
	{
		if(idx >= ATT_DEPTH)
		{
			d_printf("can't bind colour RBO to depth/stencil component\n");
			return;
		}
		fbo.attachment[idx].type = SAttachment::Texture;
		fbo.attachment[idx].active = true;
		//fbo.attachment[fbo.numColourAttachments].idx = idx;
		fbo.attachment[idx].t = to;

		if(to.descriptor.type == Texture::TEX_1D)
			glFramebufferTexture1D(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0+idx, enumToGLTextureType[to.descriptor.type], to.handle, mipmaplevel);
		if(to.descriptor.type >= Texture::TEX_2D || to.descriptor.type <  Texture::TEX_3D)
			glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0+idx, enumToGLTextureType[to.descriptor.type], to.handle, mipmaplevel);
		if(to.descriptor.type == Texture::TEX_3D)//THIS SHOULD BE glFramebufferTextureLayer
			glFramebufferTexture3D(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0+idx, enumToGLTextureType[to.descriptor.type], to.handle, mipmaplevel, 0);
	}
	else if(IS_DEPTH_TEXTURE(to.descriptor.fmt))
	{
		//maybe check if it7s already active?
		fbo.depth_attachment.type = SAttachment::Texture;
		fbo.depth_attachment.active = true;	
		fbo.depth_attachment.t = to;
		glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, enumToGLTextureType[to.descriptor.type], to.handle, 0);		
		/*
		if(fmt.nSamples == 0)
	...
	else if(IS_DEPTH_STENCIL_TEXTURE(to.descriptor.fmt))
	{
		...
		fbo.depth_attachment.active = true;	
		fbo.depth_attachment.t = to;
		glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, enumToGLTextureType[to.descriptor.type], to.handle, 0);		
	}
	//todo
...
void InactivateColorAttachment(FrameBuffer::SObject& fbo, const DrawBuffers idx)
{
	if(idx > ATT_CLR_MAX)
		return;

	fbo.attachment[idx].active = false;	
}

void RemoveColorAttachment(FrameBuffer::SObject& fbo, const DrawBuffers idx)
{
	if(idx > ATT_CLR_MAX)
		return;
	...
	//inactivate it
	if(fbo.attachment[idx].type  == SAttachment::Texture)
	{		
		if(fbo.attachment[idx].t.descriptor.type == Texture::TEX_1D)
			glFramebufferTexture1D(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0+idx, enumToGLTextureType[fbo.attachment[idx].t.descriptor.type], 0, 0);
		if(fbo.attachment[idx].t.descriptor.type >= Texture::TEX_2D || fbo.attachment[idx].t.descriptor.type <  Texture::TEX_3D)
			glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0+idx, enumToGLTextureType[fbo.attachment[idx].t.descriptor.type], 0, 0);
		if(fbo.attachment[idx].t.descriptor.type == Texture::TEX_3D)
		{	//THIS SHOULD BE glFramebufferTextureLayer
			glFramebufferTexture3D(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0+idx, enumToGLTextureType[fbo.attachment[idx].t.descriptor.type], 0, 0, 0);
		}
	}
	else
	{	//Detach
		glFramebufferRenderbuffer(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0+idx, GL_RENDERBUFFER, 0);		
	}		
}
...
void RemoveDepthAttachment(FrameBuffer::SObject& fbo)
{
	...
	fbo.depth_attachment.active = false;
	if(fbo.depth_attachment.type  == SAttachment::Texture)
	{
		glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, enumToGLTextureType[fbo.depth_attachment.t.descriptor.type], 0, 0);
	}
	else
...
void RemoveStencilAttachment(FrameBuffer::SObject& fbo)
{
	...
	fbo.stencil_attachment.active = false;
	if(fbo.stencil_attachment.type  == SAttachment::Texture)
	{
		glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, enumToGLTextureType[fbo.stencil_attachment.t.descriptor.type], 0, 0);
	}
	else
...
void RemoveDepthStencilAttachment(FrameBuffer::SObject& fbo)
{
	...
	fbo.depth_stencil_attachment.active = false;
	if(fbo.depth_stencil_attachment.type  == SAttachment::Texture)
	{
		glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, enumToGLTextureType[fbo.depth_stencil_attachment.t.descriptor.type], 0, 0);
	}
	else
...
static void checkfbo(void)
{
	switch(glCheckFramebufferStatus(GL_DRAW_FRAMEBUFFER)) 
	{
		case GL_FRAMEBUFFER_COMPLETE: 
			d_printf("GL_FRAMEBUFFER_COMPLETE\n");
			return;
			break; 			 
		case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT:
			d_printf("GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT\n");
			break;
		case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT:
			d_printf("GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT\n");
			break;			
		...
		case GL_FRAMEBUFFER_UNSUPPORTED: 
			d_printf("GL_FRAMEBUFFER_UNSUPPORTED\n");
		    break;                                                 
        default:                                               			  
			d_printf("FBO Status Error!\n");
			break;
	}
}
...
void CheckFrameBuffer(FrameBuffer::SObject& fbo)
{
	glBindFramebuffer(GL_DRAW_FRAMEBUFFER, fbo.handle);	
	checkfbo();
	fbo.initialised = true;
	glBindFramebuffer(GL_DRAW_FRAMEBUFFER, 0);	
}

//void BindFrameBuffer(FrameBuffer::Type type, FrameBuffer::SObject& fbo);
...
void SetBlendEquations(const Blend::Func clr, const Blend::Func alpha){
	glBlendEquationSeparate(enumToGLBlendFunc[clr], enumToGLBlendFunc[alpha]);
}
...
void SetBlendParams(const Blend::Param src_clr, const Blend::Param dst_clr, const Blend::Param src_alpha, const Blend::Param dst_alpha){
	glBlendFuncSeparate(
		//clr
		enumToGLBlendParam[src_clr], enumToGLBlendParam[dst_clr],
		//alpha
		enumToGLBlendParam[src_alpha], enumToGLBlendParam[dst_alpha]
	);
}
...
void EnableDepthTest(const bool set)
{
	if(set)
...
void EnableDepthWriting(const bool set)
{

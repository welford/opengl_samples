...
unsigned int tranform_feedback_buffer = 0;
static int tf_vao, tf_ab;
...
int PlatformMain( void ){
	...
	pPlatform->Create(L"Transform feedback. www.phasersonkill.com", 4, 2, 640, 640, 24, 8, 24, 8, CPlatform::MS_0);	
	Setup(pPlatform);
...
void Setup(CPlatform * const  pPlatform)
{
	...
	const char * p_outputs[] = { "position", "colour", 0 };
	...
	tranform_feedback_buffer = WRender::CreateBuffer( WRender::FEEDBACK, WRender::DYNAMIC, sizeof(float) * 3 * 2 * 3, 0 );
	WRender::BindBufferToIndex( WRender::FEEDBACK, tranform_feedback_buffer, 0 );

	//Transform feedback
	tf_vao = WRender::CreateVertexArrayObject();
	WRender::VertexAttribute va_tf[2] = {
		{ tranform_feedback_buffer, 0, 3, WRender::FLOAT, 0, sizeof(float)*6, 0,				0 },
		{ tranform_feedback_buffer, 1, 3, WRender::FLOAT, 0, sizeof(float)*6, sizeof(float)*3,	0 },		
	};		
...
{	
	...
	WRender::DeleteBuffer(tranform_feedback_buffer);
